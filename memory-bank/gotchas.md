...
- `Settings.Global.ANIMATOR_DURATION_SCALE=0` включает системный reduced-motion path: экраны Travel/Events намеренно показывают poster и не создают ExoPlayer. Перед проверкой воспроизведения на AVD верни `animator_duration_scale` в `1.0` и пересоздай Activity; доступность MP4 сама по себе этот режим не обходит.
